...
def split_proteins_imbalanced(dataset, n_min=30, n_maj=270):
    ...
    train_set = class0[:n_min] + class1[:n_maj]
    test_set  = class0[n_min:] + class1[n_maj:]
...
def eval_model(model, loader, device, num_classes):
    ...
    for data in loader:
        ...
        true = data.y.view(-1, num_classes).argmax(dim=1).cpu().numpy()
    ...
    return acc, f1_micro, f1_macro, recall
...
def run_experiment(args):
    ...
    for seed in args.seeds:
        ...
        base_train_set, base_test_set = split_proteins_imbalanced(dataset)

        print(
            "Initial train dist:",
            {i: int(c) for i, c in enumerate(get_class_stats(base_train_set)[0].tolist())}
        )
        ...
        num_sample = int(len(base_train_set) * args.aug_ratio / args.aug_num)
        print("Num sample per mix:", num_sample)
        print("Expected synthetic:", num_sample * args.aug_num)


        new_graphs = []
        for i in range(args.aug_num):
            before = len(new_graphs)
            ...
            after = len(new_graphs)
            print(f"Mix {i}: +{after-before} (total {after})")
        print("Synthetic hard-label dist:",
              Counter(onehot_to_index(g.y) for g in new_graphs))

        soft_labels = torch.stack([g.y for g in new_graphs])
        print("Soft label mean:", soft_labels.mean(dim=0).tolist())
        ...
        train_set = base_train_set + new_graphs
        ...
        for name, build in model_fns.items():
            ...
            for _ in range(args.epochs):
                ...
                acc, f1_micro, f1_macro, recall = eval_model(
                    model, test_loader, device, num_classes
                )

            results[name].append((acc, f1_micro, f1_macro, recall))
