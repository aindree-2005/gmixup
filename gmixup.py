...
def mixup_cross_entropy_loss(pred, target):
...
def train_epoch(model, loader, optimizer, device, num_classes):
    ...
    for data in loader:
        ...
        loss = mixup_cross_entropy_loss(out, y)
        loss.backward()
        optimizer.step()
...
def run_experiment(args):
    ...
    for seed in args.seeds:
        ...
        avg_nodes, _, _, median_nodes, _, _ = stat_graph(train_set)
        ...
        lam_list = np.random.uniform(
            args.lam_range[0], args.lam_range[1], size=args.aug_num
        )

        new_graphs = []
        for lam in lam_list:
            g1, g2 = random.sample(graphons, 2)
            new_graphs += two_graphons_mixup(
                [g1, g2], la=lam, num_sample=num_sample
            )

        full_dataset = new_graphs + dataset
...
def save_table(results, dataset, out_dir):
    ...
    path = os.path.join(out_dir, f"{dataset}_gmixup.csv")
